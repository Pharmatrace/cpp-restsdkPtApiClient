...
void PharmaceuticalList::fromJson(web::json::value& val)
{
    {
        m_Products.clear();
        std::vector<web::json::value> jsonArray;
        ...
        {
        for( auto& item : val[utility::conversions::to_string_t("products")].as_array() )
        {
            if(item.is_null())
            {
...
void PharmaceuticalList::setProducts(std::vector<std::shared_ptr<Pharmaceutical>> value)
{
    m_Products = value;
